<template>
  <el-radio-group v-if="tagType == 'radio'" :value="value" :disabled="disabled">
    <el-radio
      ...
      :key="key"
      :label="item.intentionClassifyName"
      @change="handleInput(item.id)"
      >{{ item.name }}</el-radio
    >
  </el-radio-group>
  ...
  <el-select
    ...
    collapse-tags
  >
    <el-option v-for="(item, key) in dictOptions" :key="key" :value="item.id" :label="item.intentionClassifyName">
    </el-option>
  </el-select>
  <!-- //  全部:搜索时可用 -->
...
<script>
import { getClassifyList } from '@/api/api'
export default {
  name: 'WDictSelectTag',
  props: {
    dictCode: String,
