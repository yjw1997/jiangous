<template>
  <div class="panel-group" v-loading="loading">
      ...
      <el-col :xs="12" :sm="12" :lg="6" class="card-panel-col">
        <div class="card-panel">
          <div class="card-panel-icon-wrapper icon-people">
            <svg-icon icon-class="zcyhzs" class-name="card-panel-icon" />
          </div>
          <div class="card-panel-description">
            <div class="card-panel-text">
              注册用户总数
            </div>
            <count-to :start-val="0" :end-val="userData.allUserCount" :duration="2600" class="card-panel-num" />
          </div>
        </div>
      </el-col>
      <el-col :xs="12" :sm="12" :lg="6" class="card-panel-col">
        <div class="card-panel">
          <div class="card-panel-icon-wrapper icon-message">
            <svg-icon icon-class="jrxzyhzs" class-name="card-panel-icon" />
          </div>
          <div class="card-panel-description">
            <div class="card-panel-text">
              今日新增用户数
            </div>
            <count-to :start-val="0" :end-val="userData.todayUserCount" :duration="2600" class="card-panel-num" />
          </div>
        </div>
      </el-col>
    </el-row>
    ...
    <el-row style="background:#fff;padding:16px 16px 0;margin-bottom:32px;">
      <div class="boxTitle">
        <div class="dashboardTitle">新注册用户数</div>
        <el-date-picker
          v-model="timeRanger"
          type="daterange"
...
import { getStatisticsQueryUserStatistics } from '@/api/platform'
...
export default {
  ...
  methods: {
    getData(data) {
      ...
      getStatisticsQueryUserStatistics(obj)
        .then((res) => {
