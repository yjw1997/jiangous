        ...
        <el-tab-pane label="订单进度" name="first">
          <div class="steps" v-if="this.dataSource.orderStatus && !this.dataSource.orderStatus.startsWith('6')">
            <el-steps :active="active" align-center finish-status="success">
              <el-step
                :title="getstepName(item.orderStatus)"
                v-for="(item, index) in dataSource.orderStatusInfos"
                :key="index"
              >
              ...
              <el-row :gutter="40">
                <el-col :span="12">
                  <div class="left">
                    ...
                    <div
                      class="status"
                      v-if="this.dataSource.orderStatus == '401' || this.dataSource.orderStatus == '302'"
                    >
                      {{ `配送方式:${dataSource.deliverType === 1 ? '商家配送' : '物流配送'}` }}
                    </div>
                    <div
                      class="status"
                      v-if="this.dataSource.orderStatus && this.dataSource.orderStatus.startsWith('6')"
                    >
                      {{ `取消原因:${this.dataSource.cancelRemark}` }}
                    </div>
                    <div class="remark" v-if="dataSource.orderStatus == '101'">
                      交易自动关闭时间剩余：
                      <FlipDown
                        :endDate="new Date(new Date().getTime() + dataSource.closeTime * 1000)"
                        @timeUp="timeOut"
                        :theme="2"
                      />
                    </div>
                    <div class="button">
                      <el-row :gutter="10" class="mb8">
                        <el-col :span="1.5">
                          <el-button type="text" plain v-if="showRemindButton()" @click="handleUserMessageRemind()"
                            >提醒买家提交凭证</el-button
                          >
                        </el-col>
                        <el-col :span="1.5">
                        ...
                        <el-col :span="1.5">
                          <el-button
                            ...
                            @click="handleUserCertificateAudit()"
                        ...
                        <el-col :span="1.5">
                          <el-button
                            type="text"
                            icon="el-icon-finished"
                            @click="handleCheckAudit()"
                            v-if="showCheckAuditButton()"
                            >查看交易凭证
                          </el-button>
                        </el-col>
                        <el-col :span="1.5">
                          <el-button
                            ...
                            v-if="showSendGoodsButton()"
                            >发货 <span v-if="this.dataSource.delivered === true">(部分发货)</span>
                          </el-button>
                        </el-col>
                        <el-col :span="1.5">
                          <el-button
                            ...
                            v-if="showCheckCodeButton()"
                            >取货码核对</el-button
                          >
                        </el-col>

                        <el-col :span="1.5">
                          <el-button type="text" icon="el-icon-document" @click="handleOrderPrint()"
                            >打印订单</el-button
                          >
                        </el-col>
                      </el-row>
      ...
      <el-tabs v-model="orderTab">
        <el-tab-pane label="订单信息" name="first">
          ...
          <el-descriptions title="订单信息">
            ...
            <template
              v-if="
                this.dataSource.orderStatus &&
                  !(this.dataSource.orderStatus.startsWith('1') || this.dataSource.orderStatus.startsWith('6'))
              "
            >
            ...
            <template
              v-if="
                this.dataSource.orderStatus &&
                  !(
                    this.dataSource.orderStatus.startsWith('1') ||
                    this.dataSource.orderStatus.startsWith('2') ||
                    this.dataSource.orderStatus.startsWith('6')
                  )
              "
            >
            ...
            <template
              v-if="
                this.dataSource.orderStatus &&
                  (this.dataSource.orderStatus.startsWith('4') || this.dataSource.orderStatus.startsWith('5'))
              "
            >
            ...
            <template v-if="this.dataSource.orderStatus && this.dataSource.orderStatus.startsWith('5')">
              <el-descriptions-item label="收货时间">{{ parseTime(dataSource.consigneeTime) }}</el-descriptions-item>
            </template>
          ...
          <div class="table">
            ...
            <el-table
              :data="this.dataSource.goodsDetails"
              tooltip-effect="dark"
              style="width: 100%"
              border
        ...
        <el-tab-pane
          label="收货和物流信息"
          name="second"
          v-if="this.dataSource.orderStatus != '302' && this.dataSource.deliverMode !== 1"
        >
          <el-descriptions title="物流信息" :column="1" :colon="false">
            ...
            <template v-else-if="dataSource.deliverType === 1">
              ...
              <el-descriptions-item label="运送方式">{{
                dataSource.deliverType === 1 ? '商家配送' : '物流配送'
              }}</el-descriptions-item>
              ...
              <el-descriptions-item label="预计送达时间">{{
                parseTime(dataSource.predictDeliverTime)
              }}</el-descriptions-item>
            </template>
            <template v-else>
              <el-descriptions-item label="收货地址">{{dataSource.consigneeName}},{{dataSource.consigneePhone}},{{ dataSource.userAddressStr }}</el-descriptions-item>
              ...
              <template v-if="dataSource.orderExpresses !== null && dataSource.orderExpresses">
                <el-descriptions-item>
                  <el-button-group>
                    <el-button
                      ...
                      :key="index"
                      @click="activeExpresses = index"
                      >{{ `包裹${index + 1}` }}</el-button
                    >
                  </el-button-group>
                </el-descriptions-item>
                ...
                <el-descriptions-item label="包裹中商品" v-if="dataSource.orderExpresses[activeExpresses]">
                  <div
                    class="goodsdata-name"
                    v-for="(goodItem, index) in dataSource.orderExpresses[activeExpresses].goodsDetails"
                    :key="index"
                  >
...
export default {
  ...
  data() {
    return {
      dataSource: { orderStatus: undefined, orderExpresses: [{}, {}] },
      form: { orderStatus: '' },
      active: 0,
      loading: false,
      activeExpresses: 0,
  ...
  computed: {
    href() {
      ...
      if (Object.keys(this.dataSource.orderExpresses[this.activeExpresses]).length === 0) {
        ...
        str =
          this.dataSource.orderExpresses[this.activeExpresses].expressCompanyUrl +
  ...
  methods: {
    init(orderNo) {
      ...
      getOrderDetail({ orderNo: orderNo })
        .then((res) => {
          ...
          // 公寓号，街道，城市，州，邮编
          this.dataSource.userAddressStr = `${this.dataSource.userAddress.houseNumber ?? '--'},${this.dataSource
            .userAddress.street ?? '--'},${this.dataSource.userAddress.city ?? '--'},${this.dataSource.userAddress
            .state ?? '--'},${this.dataSource.userAddress.postCode ?? '--'}`
          this.loading = false
    ...
    stepActive() {
      this.dataSource.orderStatusInfos.forEach((element, i) => {
        if (
          element.orderStatus == this.dataSource.orderStatus ||
          (this.dataSource.orderStatus === '102' && element.orderStatus === '101')
        ) {
          this.active = i
        }
    ...
    showSendGoodsButton() {
      if (
        ...
        (this.dataSource.orderStatus == 401 && this.dataSource.delivered === true)
      ) {
        return true
      }
      return false
